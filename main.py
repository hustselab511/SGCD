...
class Main:
    ...
    def evaluate_iter(self, dataLoader=None):
        ...
        with torch.no_grad():
            for i, data in enumerate(dataiter):
                _, loss_list, (pred_ent_matrix, pred_rel_matrix, pred_pol_matrix) = self.model('eval', **data)
    ...
    def evaluate(self, epoch=0, action='eval'):
        PATH = os.path.join(self.config.target_dir, "epoch{}_{}_{}.pth.tar").format(self.best_iter, self.best_score,
                                                                                    self.config.lang)
        ...
        result = self.relation_metric.compute('test', action,
                                              msg="f1_{:.4f}ident_{:.4f}epo_{}".format(self.best_score * 100,
                                                                                       self.best_ident * 100, epoch))

        if action == 'eval':
            self.test_score, self.test_ident, self.test_res = result
            logger.info(self.test_res)
            logger.info("Evaluate on test set, micro-F1 score: {:.4f}%, ident: {:.4f}%".format(self.test_score * 100,
                                                                                               self.test_ident * 100))

    def train(self):
        best_score, best_ident, best_iter = 0, 0, 0
        for epoch in range(best_iter, self.config.epoch_size):
            ...
            if (score > 0.425 and score > self.best_score):
                torch.save({'epoch': epoch, 'model': self.model.cpu().state_dict(), 'best_score': score,
                            'config': self.config},
                           os.path.join(self.config.target_dir,
                                        "epoch{}_{}_{}.pth.tar".format(epoch, score, self.config.lang)))
            ...
            if sum(train_losses) < 1e-3:
                if score > best_score:
                    ...
                    torch.save({'epoch': epoch, 'model': self.model.cpu().state_dict(), 'best_score': best_score,
                                'config': self.config},
                               os.path.join(self.config.target_dir,
                                            "{}_{}.pth.tar".format(self.config.lang, self.time)))
        ...
        optimizer_grouped_parameters = [
             ...
             'lr': self.config.bert_lr},
            {
                'params': [p for n, p in param_optimizer
                           ...
                           and any(e in n for e in is_expert)],
                'weight_decay': self.config.weight_decay,
                'lr': self.config.expert_lr
            },
            {
                'params': [p for n, p in param_optimizer
...

            {
                'params': [p for n, p in param_optimizer
                           if not any(nd in n for nd in no_decay)
                           and not any(b in n for b in is_bert)
                           and not any(e in n for e in is_expert)],
                'weight_decay': self.config.weight_decay,
                'lr': self.config.lr
            },
            {
    ...
    def forward(self):
        ...
        # print(f"模型参数大小约为: {param_size_MB:.2f} MB")
        ...
        with open(file_path, 'a', newline='') as csvfile:
            ...
            writer.writerow(["{:.4f}".format((self.test_score + self.test_ident) / 2 * 100),
                             ...
                             "{:.4f}".format(self.best_score * 100), "{:.4f}".format(self.best_ident * 100),
                             self.best_iter,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input_files', type=str,
                        default='train_dependent_guide_trf valid_dependent_guide_trf test_dependent_guide_trf', help='input file names')
