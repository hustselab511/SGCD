    ...
    def collate_fn(self, lst):
        if 'dep' in self.config.input_files:
            ...
            token_index, thread_length, token2speaker, reply_mask, speaker_mask, thread_mask, pieces2words, new2old, \
                triplets, pairs, entity_list, rel_list, polarity_list, thread_idxes, merged_len, ipw_mask,\
        ...
        max_lens = max([len(x) for x in ipw_mask])
        ipw_mask = [x + [0] * (max_lens - len(x)) for x in ipw_mask]
        ...
        res = {
            ...
            'pieces2words': pieces2words, 'new2old': new2old, 'thread_idxes':thread_idxes, "merged_len": merged_len, "ipw_mask": ipw_mask,
