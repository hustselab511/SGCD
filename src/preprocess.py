...
class Preprocessor:
    ...
    def read_and_split(self, file_path):
        result_array = []  # 用于存放所有分割后的字符列表

        with open(file_path, 'r', encoding='utf-8') as file:  # 打开文件
            for line in file:  # 遍历文件中的每一行
                # 使用split方法按空格分割字符串，并去除每部分的前后空白字符
                chars = [char.strip() for char in line.split()]
                result_array.append(chars)  # 将分割后的字符列表添加到结果数组中

        result_array = [self.tokenizer.convert_tokens_to_ids(x) for x in result_array]

        return result_array

    def split_words_to_tokens(self, file_path):
        char_array = []  # 用于存放所有词语分割后的字符

        with open(file_path, 'r', encoding='utf-8') as file:  # 打开文件
            for line in file:  # 遍历文件中的每一行
                word = line.strip()  # 去除首尾的空白字符（包括'\n'）
                chars = list(word)  # 将词语转为字符列表
                char_array.append(chars)  # 将字符列表添加到结果数组中

        char_array = [self.tokenizer.convert_tokens_to_ids(x) for x in char_array]

        return char_array

    def parse_file_to_2d_list(self, file_path):
        two_d_list = []
        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                items = line.strip().split(',')
                two_d_list.append(items)
        return two_d_list

    def parse_file_and_pad(self, file_path, max_length=None):
        tokens_list = []
        all_lengths = []

        # with open(file_path, 'r', encoding='utf-8') as file:
        #     for line in file:
        #         # 去除行末的换行符，并按照空格分隔每一行的内容
        #         items = line.strip().split()
        #         all_lengths.append(len(items))
        #
        # if max_length is None:
        #     max_length = max(all_lengths) + 2

        with open(file_path, 'r', encoding='utf-8') as file:
            for line in file:
                items = line.strip().split()
                # items_with_special_tokens = [self.config.cls] + items + [self.config.sep]
                # padding_size = max_length - len(items_with_special_tokens)
                # padded_items = items_with_special_tokens + [self.config.pad] * padding_size
                tokens_list.append(items)
    ...
    def mark_sentences(self, sentences, keywords):
        # 初始化标记列表，默认全部为0
        markers = [0] * len(keywords)

        # 遍历每个句子
        for sentence in sentences:
            sentence = ''.join(word.replace('#', '') for word in sentence)
            # 对于每一个关键词组
            for i, key_group in enumerate(keywords):
                # 如果该组内的任意一个词出现在句子中
                if any(keyword.lower() in sentence.lower() for keyword in key_group):
                    # 将对应的标记置为1
                    markers[i] = 1
    ...
    def transform2indices(self, dataset, mode='train'):
        ...
        for document in dataset:
            ...
            global_ipw = document['global_ipw']
            ...
            merged_len = len(merged_input_ids)


            speakers_len = max(speakers) + 1
            speakers_ipw = [0] * speakers_len
            for speaker in speakers:
                speakers_ipw[speaker] += 1
            all_w = 0
            for i, w in enumerate(speakers_ipw):
                if w > 0:
                    w = math.exp(1.0 / w)
                    all_w += w
                    speakers_ipw[i] = w
            ...
            for i, w in enumerate(speakers_ipw):
                speakers_ipw[i] = speakers_len * w / all_w

            thread_ipw = [1] * len(merged_sentence_length)
            all_w = 0
            for i, ipw in enumerate(thread_ipw):
                ipw = math.exp(1.0 / ipw + thread_nums[i + 1])
                all_w += ipw
                thread_ipw[i] = ipw

            for i, ipw in enumerate(thread_ipw):
                thread_ipw[i] = len(thread_ipw) * ipw / all_w

            ipw_mask = [global_ipw] * len(sentence_length)

            for i, speaker in enumerate(speakers):
                ipw_mask[i] *= speakers_ipw[speaker]
            for i, ipw in enumerate(thread_ipw):
                for k in range(thread_range[i+1][0], thread_range[i+1][1]):
                    ipw_mask[k] *= ipw

            for i, lens in enumerate(sentence_length):
                ipw_mask[i] = [ipw_mask[i]] * lens
            ipw_mask = [item for sublist in ipw_mask for item in sublist]
            ...
            if 'dep' in mode:
                ...
                res.append((doc_id, speakers, input_ids, input_masks, input_segments, sentence_length, nsentence_ids, utterance_index, token_index, 
                            ...
                            new_triplets, pairs, entity_lists, relation_lists, polarity_lists, thread_idxes, merged_len, ipw_mask,
                ...
                res.append((doc_id, speakers, input_ids, input_masks, input_segments, sentence_length, nsentence_ids, utterance_index, token_index, 
                        thread_length, token2speaker, reply_mask, speaker_masks, thread_masks, pieces2words, new2old,
                        new_triplets, pairs, entity_lists, relation_lists, polarity_lists, thread_idxes, ipw_mask,
                        merged_input_ids, merged_input_masks, merged_input_segments, merged_sentence_length))
