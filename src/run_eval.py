...
class Template:
    ...
    def post_process(self, line, key='quad'):
        ...
        if key in ['intra', 'inter']:
            ...
            return res
        if key in ['cross-1', 'cross-2', 'cross-3', 'cross-ge2']:
            for comb in line['triplets']:
                if any(w == -1 for w in comb[:6]): continue
                comb[6] = comb[6] if comb[6] in ['pos', 'neg'] else 'other'
                distance = get_utterance_distance(line['sentence_ids'], line['dis_matrix'], comb[0], comb[2], comb[4])
                ...
                if key == 'cross-ge2' and distance <2: continue
                res.append(tuple(comb[:7]))
            return res
    ...
    def forward(self, print_line=False):
        ...
        scores = []
        res = '\nItem\tPrec.\tRec.\tF1\n'
        items = ['targets', 'aspects', 'opinions', 'ta', 'to', 'ao', 'quad', 'iden', 'intra', 'inter', 'cross-1', 'cross-ge2', 'cross-3']
        item_name = ['Target', 'Aspect', 'Opinion', 'TA', 'TO', 'AO', 'Micro', 'Iden', 'Intra', 'Inter', 'Cross-1', 'Cross-ge2', 'Cross-3']
...
def process_res(res):
    ...
    res = res.split("\n")
    for line in res:
        if line.startswith("Item") or '------------------------------' in line:
            continue
        line = line.split("\t")
...
if __name__ == '__main__':
    ...
    args.gold_file = 'data/dataset/jsons_zh/test.json'
    # w/o global
    # paths = [
    #     "data/save/pred_zh_test_f1_42.5451ident_47.4834epo_31.json",
    #     "data/save/pred_zh_test_f1_41.8288ident_48.0545epo_38.json",
    #     "data/save/pred_zh_test_f1_44.0313ident_49.1194epo_29.json",
    #     "data/save/pred_zh_test_f1_43.7276ident_48.2079epo_24.json",
    #     "data/save/pred_zh_test_f1_42.5612ident_48.2109epo_27.json",
    # ]

    # w/o thread
    paths = [
        "data/save/pred_zh_test_f1_41.9355ident_46.8691epo_26.json",
        "data/save/pred_zh_test_f1_41.6586ident_47.6374epo_38.json",
        "data/save/pred_zh_test_f1_42.5366ident_48.7805epo_37.json",
        "data/save/pred_zh_test_f1_43.2892ident_48.0151epo_35.json",
        "data/save/pred_zh_test_f1_42.2812ident_47.9843epo_35.json",
    ]
    # args.gold_file = 'data/dataset/jsons_en/test.json'
    # paths = read_path("data/save/en_42-46/")
    # args.gold_file = 'data/dataset/jsons_zh/test.json'
    # paths = read_path("data/save/zh_top2+global+3attn/")
    for path in paths:
        print(path)
        args.pred_file = path
        template = Template(args)
        micro_score, iden_score, res_line = template.forward(True)
        res_line = process_res(res_line)
        for key, value in res_line.items():
            if key not in res:
                res[key] = []
            res[key].append(float(value))
    for key, value in res.items():
        print(key, np.mean(value).round(2))